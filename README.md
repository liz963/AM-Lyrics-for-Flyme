...
v1.3.15 顺带做过一轮**纯可读性重构**（行为零变化）：把原本 689 行、22 个散落状态字段的
`BackgroundLyrics.kt` 拆成「编排层 + 两个纯职责组件」，划清边界，避免继续长成面条代码。

```
app/src/main/java/com/amlyric/flyme/
├─ HookEntry.kt                  LSPosed 入口（onPackageLoaded → 安装 Hook）
├─ XLog.kt / Settings.kt         日志与开关
├─ hook/
│  ├─ AppleMusicHooks.kt         全部 Hook 安装点（取词 / 句柄捕获 / 播放状态 / 载波 / 前台逐行回调）
│  └─ NativeLyricsParser.kt      原生歌词对象 → 纯文本（反射 + HTML 清洗）
├─ core/
│  ├─ BackgroundLyrics.kt  434   ★ 后台轮询编排层：唯一调度者、唯一改状态栏的人
│  ├─ SongTitleGate.kt     193   ★ 歌名展示闸门：纯状态机，只出决策、不做动作
│  ├─ LyricsEngineDriver.kt 151  ★ 官方引擎反射驱动：唯一持有混淆名 / SAM / Proxy 细节
│  ├─ LyricController.kt   135   对外状态门面（切歌 / 加载 / 暂停 / 无歌词 / 静默）
│  ├─ LyricsLoader.kt      282   取词：Hook 捕获 + 主动 loadLyrics + 重试
│  └─ LrcParser.kt          79   本地 LRC 兜底
├─ flyme/FlymeStatusBarLyric.kt 281  载波通道：Ticker 注入宿主媒体通知 / 清除
...
| 组件 | 只负责 | 明确不负责 |
|------|--------|-----------|
| `BackgroundLyrics` | 轮询节拍、读宿主播放进度 / MediaItem、把三方串起来、**所有**状态栏推送 | 不判断"该不该推歌名"（问闸门）、不碰反射细节（问驱动） |
| `SongTitleGate` | 歌名锁定 / 无歌词静默 / **最短展示护栏** / 重播重武装的**决策**，`evaluate()` 返回 `None / Release / NoLyrics` | 不发通知、不打日志、不知道引擎和宿主的存在 |
| `LyricsEngineDriver` | `ensureReady()` / `drive(ptr, queryPos) → 下一事件绝对位置` / `peek(ptr, atPos) → 文本` | 不做调度、不管歌名锁定、不知道 `LyricController` |

`BackgroundLyrics.onTick()` 的顺序是**约定**，改动请保持：读位置 → 处理 MediaItem（切歌/重播/推歌名/取句柄）
...
> 依旧把歌名顶掉——这就是"歌名一闪而过"反复修不好的真正原因。任何新增的抑制条件
> （如静默、暂停冻结）都要经 `BackgroundLyrics.onForegroundLine()` 走同一把闸门。
...
| 无歌词（从头播放） | 歌名-歌手显示 **8 秒**后**清空状态栏**，本首剩余时间**不再推送任何内容**（标题 / 歌词 / 占位一概不推）；若期间引擎其实报出了歌词事件，自动解除静默 |
| 前奏 / 间奏 | 当前无对应歌词行时显示 `♪ ♪ ♪` |
...
- **v1.3.16**：**修复「歌名一闪而过」的根因**——真机日志证实官方引擎 `processEvents` 的返回值**不是「下一行」而是「下一事件」**（行内字级事件也会被报出来，实测提前量小到 2ms）。当第一句来得很早、或歌词句柄迟到（拿到 ptr 时已进入第一句内部）时，算出的释放点会落在**过去**，于是锁定在下一 tick 就被解除，歌名只活一两百毫秒。现在释放点加了下限「推出位置 + 3 秒」：无论首句时间多离谱，歌名至少显示 3 秒；前奏正常的歌曲（首句在 4 秒之后）释放点完全不受影响。另修：位置读取失败原会返回 0，与「真的回到开头」不可区分，会误清歌名锁定（改为读取失败则跳过本 tick）；顺带清理死代码与重复状态。
- **v1.3.15**：**无歌词歌曲的处理**——部分歌曲宿主根本不产出歌词（日志表现为 `loadLyrics timeout ... no ptr after 15s`，此时连歌词句柄都没有，锁定永远等不到第一句、歌名会卡住整曲）。改为：歌名-歌手显示 **8 秒**（`TITLE_HOLD_MAX_MS`）后**清空状态栏**，本首剩余时间内**不再推送任何内容**（标题/歌词/「暂无歌词」占位一概不推）。若期间引擎其实报出了歌词事件（说明只是加载慢），自动解除静默、恢复正常推送。
